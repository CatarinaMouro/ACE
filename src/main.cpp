...
int media_right(){
  int sum=valores_right[0];
  for(int i=1; i<N; i++){
    sum = sum + valores_right[i];
  }
  return sum/N;
} 

int media_left(){
  int sum=valores_left[0];
  for(int i=1; i<N; i++){
    sum = sum + valores_left[i];
  }
  return sum/N;
} 
